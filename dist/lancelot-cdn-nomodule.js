(() => {
  ...
  var Polygon = class extends FixedDrawable {
    _points;
    constructor(params) {
      super(params);
  ...
  var RoundedRect = class extends Polygon {
    _borderRadius;
    constructor(params) {
      super(params);
  ...
  var Body = class extends Component {
    ...
    handleBehavior() {
      ...
      for (let behavior of this._behavior) {
        const entities = (controller === void 0 ? this.parent.scene.world.findNearWithoutQuadtree([this.position.x, this.position.y], [boundingRect.width, boundingRect.height]) : controller.findNearby(boundingRect.width, boundingRect.height)).filter((e) => {
  ...
  var Polygon2 = class extends Body {
    _points;
    constructor(params) {
      super(params);
      this._points = params.points;
  ...
  var Box = class extends Polygon2 {
    _width;
    _height;
    constructor(params) {
      super(params);
  ...
  var detectCollision = (b1, b2) => {
    ...
    } else if (b1 instanceof Polygon2 && b2 instanceof Polygon2) {
      return detectCollisionPolyVsPoly(b1, b2);
    } else if (b1 instanceof Ball && b2 instanceof Polygon2) {
      return detectCollisionBallVsPoly(b1, b2);
    } else if (b1 instanceof Polygon2 && b2 instanceof Ball) {
      return detectCollisionBallVsPoly(b2, b1);
    } else if (b1 instanceof Ray && b2 instanceof Polygon2) {
      return detectCollisionRayVsPoly(b1, b2);
    } else if (b1 instanceof Polygon2 && b2 instanceof Ray) {
  ...
  var detectCollisionPolyVsPoly = (b1, b2) => {
    ...
    const normals1 = Polygon2.getFaceNormals(verts1);
    const normals2 = Polygon2.getFaceNormals(verts2);
    let e1SupportPoints = [];
    for (let i = 0; i < normals1.length; i++) {
      let spInfo = Polygon2.findSupportPoint(verts2, normals1[i].clone().mult(-1), verts1[i]);
    ...
    for (let i = 0; i < normals2.length; i++) {
      let spInfo = Polygon2.findSupportPoint(verts1, normals2[i].clone().mult(-1), verts2[i]);
  ...
  var detectCollisionBallVsPoly = (b1, b2) => {
    const verts2 = b2.getComputedVertices();
    const normals = Polygon2.getFaceNormals(verts2);
    let e1SupportPoints = [];
    ...
    let info = Polygon2.findSupportPoint(verts2, normal.clone(), b1.position.clone().add(normal.clone().mult(-b1.radius)));
    if (info.sp == void 0)
      return { collide: false };
    info.n = normal.clone();
  ...
  var resolveCollision = (b1, b2, options) => {
    if (b1 instanceof Ball && b2 instanceof Polygon2) {
      [b1, b2] = [b2, b1];
    }
